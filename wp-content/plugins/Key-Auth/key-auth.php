...
class JSON_Key_Auth {
	...
	public static function authHandler( $user ) {
		...
		if ( !isset( $httpHeaders['HTTP_X_API_KEY'] ) || !isset( $httpHeaders['HTTP_X_API_TIMESTAMP'] ) || !isset( $httpHeaders['HTTP_X_API_SIGNATURE'] ) ) {
			return $user;
		}

		
		$user_id = self::findUserIdByKey( $httpHeaders['HTTP_X_API_KEY'] );
		...
		$signature_args = array(
			'api_key' => $httpHeaders['HTTP_X_API_KEY'],
			'timestamp' => $httpHeaders['HTTP_X_API_TIMESTAMP'],
		...
		$signature = $httpHeaders['HTTP_X_API_SIGNATURE'];
	...
	public static function generateSignature( $args, $secret ) {
		return md5(stripslashes(json_encode( $args )) . $secret);
	}
...
function jsrk_show_user_api_fields( $user ) { ?>
	...
	<table class="form-table">
		...
		<tr>
			...
			<td>
				<input type="text" name="jsrk_apikey" id="jsrka-apikey" value="<?php echo get_user_meta( $user->ID, 'json_api_key', TRUE); ?>" class="regular-text" /> <input type="button" class="button button-secondary" value="Generate" onclick="jsonRestKeyAuth.generateAPIKeyToField(jQuery('#jsrka-apikey')); return false;"><br />
			</td>
		</tr>
		<tr>
			...
			<td>
				<input type="text" name="jsrk_shared_secret" id="jsrka-apisecret" value="" class="regular-text" /><input type="button" class="button button-secondary" value="Generate" onclick="jsonRestKeyAuth.generateSharedSecret(jQuery('#jsrka-apisecret')); return false;"><br />
			</td>
		</tr>
